(ns uraniumops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (UraniumOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the UraniumThoriumMiningGovernor
  ...
  human operator/radiation-safety officer. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [uraniumops.advisor :as advisor]
            [uraniumops.governor :as governor]
...
(defn build
  ...
  `uraniumops.store/Store`).
  opts:
    :advisor      -- a `uraniumops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; UraniumOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; UraniumThoriumMiningGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
